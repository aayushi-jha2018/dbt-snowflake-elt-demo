...
`-- models/
    |-- staging/
    |   |-- _staging.yml       # sources + column tests
    |   |-- stg_customers.sql
    |   `-- stg_orders.sql
    `-- marts/
...
- **Testing**: `_staging.yml` and `_marts.yml` enforce not-null, uniqueness, referential integrity, and accepted-value constraints as part of `dbt test`.
